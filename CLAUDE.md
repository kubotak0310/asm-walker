...
- **ARM ABI 可視化**:
  ...
  - StepController: 関数リターン時に「funcName() 実行完了 — r0 = X (N)」バナーを表示（プリセット・自由入力モード）
...
- **ARM ABI バッジ表示**（フェーズ3追加）:
  ...
  - Cソースなし（自由入力モード）・可変長引数・関数ポインタ引数の場合はバッジ非表示
  - 戻り値バッジが引数バッジより優先される
...
- 関数リターン時の「実行完了」バナーはコンパイルモードでは非表示（CCompilePanelバーで代替）
