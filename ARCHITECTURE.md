...
Godbolt API（CORS許可済み・開発時・本番時ともに同一）
...
**CORS**: Godbolt API は `Access-Control-Allow-Origin: *` を返すため、ブラウザから直接 POST 可能。プロキシ不要。
...
Godbolt API が CORS を許可しているためプロキシ不要。静的サイトとして Vercel にデプロイするだけで動作する。
